...
// Fix all navigation links
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Fix specific section IDs that might be missing
        const sections = ['home', 'about', 'services', 'portfolio', 'team', 'membership', 'contact'];
        sections.forEach(sectionId => {
            if (!document.getElementById(sectionId)) {
                // Find elements with similar IDs
                const similarElements = document.querySelectorAll(`[id*="${sectionId}"]`);
                if (similarElements.length > 0) {
                    // Add the main ID to the first matching element
                    similarElements[0].id = sectionId;
                }
            }
        });
    });